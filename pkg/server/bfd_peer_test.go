...
// Test_RxPacketLeavingUpRefloorsAndStartsNewPollSequence pins RFC 5880 Section 6.8.3: leaving
// Up re-applies the not-Up floor. Since that changes bfd.DesiredMinTxInterval away from the
// configured value, it initiates a new Poll Sequence.
func Test_RxPacketLeavingUpRefloorsAndStartsNewPollSequence(t *testing.T) {
	assert := assert.New(t)
...
// Test_RxPacketFinalThenDownOrdering pins the ordering of the Final check in rxPacket: it must
// run before the state-transition switch. A single packet that carries Final while also
// signaling Down has to terminate the OLD Poll Sequence first; the Down transition that the
// same packet then triggers starts a NEW one. So pollSequence ends up true, not false — this
// test would observe false if the Final check were moved after the state switch.
func Test_RxPacketFinalThenDownOrdering(t *testing.T) {
	assert := assert.New(t)
...
// the configured value. The observed
...
// either the P-bit propagation in tx() or the eventTx.Reset in setDesiredMinTx is reverted:
// without the P bit, the periodic Up packets below would carry Poll == false; without the
// Reset, the ticker stays at the 1s floor and the three Up packets arrive far more than 1.5s
// apart instead of the ~400ms (immediate packet plus two 200ms ticks) the fix produces.
...
// and setDesiredMinTx must re-arm the timer for it, because on this path nothing
// else will. A timer left at the Up cadence fires once more at the fast interval,
// one packet closer than Section 6.8.7 allows, before tx() corrects it.
func Test_ExpiryRefloorsTxPacing(t *testing.T) {
	...
	// The floor is back and the next packet is at lastTx + the jittered 1s, at
	// least 550ms away (lastTx was at most one 200ms period before expiry, and
	// the jittered floor is at least 750ms). Nothing may go out in the next
